...
Counterexample: ["1", "1"]
Interpretation: It is raining, it is sprinkling
Explanation: affirm_disjunct states that if it is raining, then it cannot be sprinkling (¬isSprinkling). Since in
this case both conditions are true (it’s raining and sprinkling), this interpretation would make the expression false.
...
-/
def cxs1 : List BoolInterp := findCounterexamples deny_antecedent
#eval! interpStringsFromInterps cxs1 2

def cxs2 : List BoolInterp := findCounterexamples affirm_consequent
...
Counterexample: ["1", "1"]
Interpretation: It is raining, it is sprinkling
Explanation: deny_antecedent claims that if it's sprinkling, it must not be raining. However, in this interpretation,
it is both sprinkling and raining. Thus, in this case, the expression would be false.

Counterexample: ["1", "0"]
Interpretation: It is raining, it is not sprinkling
Explanation: affirm_consequent claims that if it’s raining, then it must be sprinkling.
However, in this interpretation, it is raining but not sprinkling. Thus, in this case, the expression would be false.
